class Piece {
  ...
  getMovesRight() {
    ...
    const movesRight = [];
    for (let move = this.position + 1; move <= parseInt(num[0] + "8"); move++)
      movesRight.push(move);
    return movesRight;
  }
  ...
  getMovesLeft() {
    const num = this.position + "";
    const movesLeft = [];
    for (let move = this.position - 1; move >= parseInt(num[0] + "1"); move--)
      movesLeft.push(move);
    return movesLeft;
  }
